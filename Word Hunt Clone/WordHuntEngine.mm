...
public:
    ...
    std::vector<std::string> generateBoard(uint64_t seed) const {
        static constexpr char distribution[] = "EEEEEEEEEEEEAAAAAAAAAIIIIIIIOOOOOONNNNNNRRRRRRTTTTTTLLLLSSSSSSUUUUDDDDGGGBBCCCCMMPPFFHHHVVWWYYKKKJXQZ";
        static constexpr uint32_t distributionCount = static_cast<uint32_t>(sizeof(distribution) - 1);

        Xoshiro256StarStar rng(seed);
        std::vector<std::string> board;
        board.reserve(kBoardSize);
        for (int index = 0; index < kBoardSize; ++index) {
            char letter = distribution[rng.nextBounded(distributionCount)];
            board.emplace_back(1, letter);
        }
        return board;
    }
